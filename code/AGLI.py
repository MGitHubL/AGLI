...
from dataset import AGLIDataSet
...
class AGLI:
    def __init__(self, directed=False):
        ...
        if torch.cuda.is_available():
            with torch.cuda.device(DID):
                node_emb = AGLI.position_encoding_(self.node_dim, self.emb_size)
                ...
                self.delta_a = AGLI.truncated_normal_(self.delta_a, tensor=(self.delta_a))
                self.delta_c = Variable((torch.zeros(self.node_dim) + 1.).type(FType).cuda(), requires_grad=True)
                self.delta_c = AGLI.truncated_normal_(self.delta_c, tensor=(self.delta_c))
                self.delta_t = Variable((torch.zeros(self.node_dim) + 1.).type(FType).cuda(), requires_grad=True)
                self.delta_t = AGLI.truncated_normal_(self.delta_t, tensor=(self.delta_t))

                self.w_node = Variable((torch.zeros(4, self.emb_size, self.emb_size) + 1.).type(FType).cuda(),
                                       requires_grad=True)
                self.w_node = AGLI.truncated_normal_(self.w_node, tensor=(self.w_node))
                self.w_neighbor = Variable((torch.zeros(4, self.emb_size, self.emb_size) + 1.).type(FType).cuda(),
                                           requires_grad=True)
                self.w_neighbor = AGLI.truncated_normal_(self.w_neighbor, tensor=(self.w_neighbor))
                self.w_network = Variable((torch.zeros(4, self.emb_size, self.emb_size) + 1.).type(FType).cuda(),
                                          requires_grad=True)
                self.w_network = AGLI.truncated_normal_(self.w_network, tensor=(self.w_network))
                self.b = Variable((torch.zeros(4, self.node_dim, self.emb_size) + 1.).type(FType).cuda(),
                                  requires_grad=True)
                self.b = AGLI.truncated_normal_(self.b, tensor=(self.b))
        ...
        self.opt = torch.optim.Adam(lr=self.lr, params=[self.delta_a, self.delta_c, self.delta_t, self.w_node,
    ...
    def active_status(self, s_nodes, s_cumulative_affect, s_node_emb, t_times):
        flag_judgment = torch.eq(self.active_flag[s_nodes], self.zero_judgment) + 0  # [batch]
        affect_judgment = torch.gt(s_cumulative_affect, self.affect_threshold) + 0  # [batch]
    ...
    def forward(self, s_nodes, t_nodes, t_times, n_nodes, h_nodes, h_times, h_time_mask):
        ...
        delta_c = self.delta_c.index_select(0, Variable(s_nodes.view(-1))).unsqueeze(1)  # [batch_size, 1]
        ...
        attenuation_factor = torch.mul(hist_index.unsqueeze(0), delta_c)
        affect_per_neighbor = torch.mul(s_node_emb.unsqueeze(1), h_node_emb).sum(dim=-1, keepdim=False)
        affect_sum = affect_per_neighbor.sum(dim=-1, keepdim=True) + 1e-6
        affect_weight = affect_per_neighbor / affect_sum

        d_time = torch.abs(t_times.unsqueeze(1) - h_times)
        hawkes_value = torch.exp(delta_t * Variable(d_time))

        neighborhood_param = (attenuation_factor * affect_weight * hawkes_value * Variable(h_time_mask))
        ...
        s_cumulative_affect = torch.abs(affect_sum).squeeze(-1)
        self.active_status(s_nodes, s_cumulative_affect, s_node_emb, t_times)
